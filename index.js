...
app.get('/api/get_reviews', (req, res) => {
    db.query(`SELECT
            ...
            movie_reviews.id AS movie_review_id, CONCAT(movie_reviews.first_name, ' ', movie_reviews.last_name) AS full_name, movie_reviews.email_address, movie_reviews.review, movie_reviews.rating
        FROM movie_reviews
...
app.post('/api/create_rating', (req, res) => {
    ...
    db.query(`INSERT INTO movie_reviews (movie_id, first_name, last_name, email_address, review, rating, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW());`, [form_data.movie_id, form_data.first_name, form_data.last_name, form_data.email_address, form_data.review, form_data.rating], (err, result) => {
            res.send(result);
        })
});
